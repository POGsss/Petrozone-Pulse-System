...
The Catalog Management module manages the master list of service/product items offered by the business. Catalog items are **global** (no branch scoping) and serve as the building blocks for job orders — when creating a JO, users select items from the catalog and pricing is resolved from pricing matrices. Each catalog item can be linked to one or more inventory items, forming a template of materials consumed when that service is performed.

### Key Business Rules

1. **Global items** — all catalog items are visible to all users regardless of branch assignment. There is no branch scoping or global toggle.
2. **Inventory links** — catalog items can be linked to inventory items (template associations). These links define which materials are consumed when the catalog item is added to a job order. Links can be managed during creation (Add modal) or editing (Edit modal).
3. **No base price** — catalog items do not have a base price. Pricing is handled entirely by the Pricing Matrix module (light/heavy/extra\_heavy prices by vehicle class).
4. **No item types** — catalog items have no type classification (no service/product/package). They are all generic items.
...
> **Note:** T (Technician) does not have access to the Catalog page at all. R (Receptionist) is view-only.

### API Endpoints

| Method   | Endpoint                                          | Description                          |
| -------- | ------------------------------------------------- | ------------------------------------ |
| `GET`    | `/api/catalog`                                    | List catalog items (filtered)        |
...
| `GET`    | `/api/catalog/:itemId/inventory-links`            | Get linked inventory items           |
| `POST`   | `/api/catalog/:itemId/inventory-links`            | Link an inventory item               |
| `DELETE` | `/api/catalog/:itemId/inventory-links/:linkId`    | Remove an inventory link             |
...
### Test 2 — Create Catalog Item (with Inventory Links)

**Goal:** Verify a catalog item can be created with inventory links in the same modal.
...
### Test 3 — View Catalog Item Details

**Goal:** Verify the view modal shows all item data including linked inventory.

1. Click on a catalog card (not on the Edit/Delete buttons) to open the view modal
2. Verify the **"Catalog Item Details"** modal shows:
   ...
   - ✅ **"Linked Inventory"** section:
     ...
     - If no links: `"No inventory items linked to this catalog item."`
   - ✅ **"Timestamps"** section:
     - Created At and Updated At dates

---

### Test 4 — Edit Catalog Item (with Live Inventory Management)

**Goal:** Verify catalog item details and inventory links can be edited.

1. Click the **Edit** (pencil icon) button on a catalog card
...
1. Create more than 12 catalog items
...
### Test 8 — Delete Catalog Item (No FK References — Hard Delete)
...
### Test 9 — Delete Catalog Item (FK Reference Exists — Soft Delete / Deactivation)

**Goal:** Verify a catalog item used in job orders is deactivated instead of deleted.

1. Create a job order that references a catalog item (via Job Order Management)
2. Return to Catalog and click **Delete** on that catalog item
3. Confirm the deletion
4. Verify:
   - ✅ Toast (info): `"Catalog item is referenced by other records and has been deactivated instead of deleted."`
...
1. **Log in as R (Receptionist):**
   ...
   - ✅ **No** "Add New Catalog" button visible
   - ✅ **No** Edit or Delete buttons on cards
2. **Log in as T (Technician):**
   - ✅ Catalog should **not** appear in the sidebar navigation
3. **Log in as HM, POC, or JS:**
   - ✅ "Add New Catalog" button is visible
...
1. Perform create, update, delete operations on catalog items
2. Navigate to **Audit Logs**
3. Verify entries exist for:
   - ✅ Catalog item creation (action: CREATE, entity_type: catalog_items)
