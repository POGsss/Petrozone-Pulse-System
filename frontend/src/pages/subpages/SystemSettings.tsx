...
export function SystemSettings() {
    ...
    const [customColorValue, setCustomColorValue] = useState(
        /^#([0-9a-fA-F]{6})$/.test(settings.primaryColor) ? settings.primaryColor : "#0f766e"
    );

    const visibleColorPresets = useMemo(
        () => COLOR_PRESETS.filter((preset) => !preset.name.toLowerCase().includes("teal")),
        []
    );

    const isPresetSelected = useMemo(
        () => visibleColorPresets.some((preset) => preset.value.toLowerCase() === draft.primaryColor.toLowerCase()),
        [visibleColorPresets, draft.primaryColor]
    );
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white border border-neutral-200 rounded-xl p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="p-2.5 bg-neutral-100 rounded-lg">
                            <LuClipboardList className="w-5 h-5 text-neutral-950" />
                        </div>
                        <div>
                            <h4 className="font-medium text-neutral-950">Table Density</h4>
                            <p className="text-sm text-neutral-900">
                                {draft.tableDensity === "compact"
                                    ? "Compact rows are selected"
                                    : "Comfortable rows are selected"}
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        className={`relative w-14 h-7 rounded-full transition-colors duration-300 ${draft.tableDensity === "compact" ? "bg-primary" : "bg-neutral-300"
                            }`}
                    >
                        <span
            ...
            <div className="bg-white border border-neutral-200 rounded-xl p-6">
                <div className="flex items-center gap-4 mb-4">
                    <div className="p-2.5 bg-neutral-100 rounded-lg">
                        <LuPalette className="w-5 h-5 text-neutral-950" />
                    </div>
                    <div>
                        <h4 className="font-medium text-neutral-950">Primary Color</h4>
                        <p className="text-sm text-neutral-900">
                            Choose the accent color used throughout the system.
                        </p>
                    </div>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            patchDraft({ primaryColor: customColorValue });
                            customColorInputRef.current?.click();
                        }}
                        className={`relative flex items-center gap-3 px-3 py-3 rounded-xl border-2 transition-all ${!isPresetSelected
                                ? "border-primary bg-primary-100"
                                : "border-neutral-200 hover:border-neutral-400"
                            }`}
                    >
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                setCustomColorValue(next);
                                patchDraft({ primaryColor: next });
                            }}
                            className="sr-only"
                            tabIndex={-1}
                        ...
                        <span
                            className="w-8 h-8 rounded-full shrink-0 ring-2 ring-white shadow-sm"
                            style={{ background: "conic-gradient(#ff0000, #ffa500, #ffff00, #00ff00, #00ffff, #0000ff, #8b00ff, #ff0000)" }}
                        />
                        <span className="text-sm font-medium text-neutral-950 truncate hidden sm:block">Custom</span>
                    </button>
                </div>
            </div>
