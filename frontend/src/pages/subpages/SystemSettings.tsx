    ...
    <div className="space-y-6">
          ...
          {COLOR_PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-3 px-3 py-3 rounded-xl border-2 transition-all ${
                  isActive
              ...
              >
                ...
                />
                <span className="text-sm font-medium text-neutral-950 truncate">{preset.name}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-colors duration-300 ${
              draft.sidebarCollapsed ? "bg-primary" : "bg-neutral-300"
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform duration-300 ${
                draft.sidebarCollapsed ? "translate-x-7" : "translate-x-0"
              }`}
            />
          </button>
        </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h4 className="font-medium text-neutral-950">UI Size</h4>
          ...
          {FONT_SIZE_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs text-neutral-900">{option.description}</span>
              </button>
            );
          })}
        </div>
